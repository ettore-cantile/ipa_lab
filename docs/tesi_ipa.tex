...
Il traffico UDP sulla porta 9999 attraversa regolarmente la rete emulata. Nelle prove effettuate, un invio da \texttt{darmstadt} al loopback di \texttt{aachen}, a tre salti di distanza, è stato consegnato integralmente. Prima di avviare i test occorre però attendere la convergenza di OSPF. Nella dorsale di cinquanta nodi e ottantotto link, il protocollo impiega circa un minuto e mezzo a popolare le tabelle di instradamento; fino ad allora sono raggiungibili soltanto le sottoreti direttamente connesse. I test avviati subito dopo l'accensione del laboratorio falliscono quindi per questo motivo, non per un limite dell'emulatore.

La verifica sistematica di correttezza usa comunque \texttt{BPF\_PROG\_TEST\_RUN}, perché permette di eseguire prove controllate e ripetibili. La primitiva esegue un programma XDP già caricato su un pacchetto sintetico direttamente nel kernel e restituisce il verdetto e una misura della durata, senza attraversare la scheda di rete. Si possono così controllare TTL, stato dei link e \texttt{model\_id}, isolando il test dal traffico della rete emulata. Con pacchetti reali, ogni caso richiederebbe invece di predisporre lo stato dell'intera topologia. Il limite è che, in questa modalità, il redirect non viene realmente eseguito.
...
I programmi eBPF leggono la mappa a ogni pacchetto, senza essere ricaricati. Quando un'interfaccia cade, la feature passa a 0 al polling successivo, eseguito ogni mezzo secondo. L'argmax può così scegliere un'altra uscita: è il meccanismo indicato nella documentazione come \emph{fast-reroute}. All'avvio vengono impostati a 1 soltanto gli slot associati a un'interfaccia realmente presente sul nodo; quelli senza interfaccia partono da 0 e restano a 0. Nei log, il monitor distingue un'uscita strutturalmente assente da un link realmente caduto.
...
La misura delle tre pipeline usa \texttt{BPF\_PROG\_TEST\_RUN} per le ragioni discusse nella Sezione~\ref{sec:lab}. Il confronto include una quarta configurazione, la \emph{baseline}: un programma con lo stesso parsing del dispatcher e lo stesso redirect finale, ma senza tail call né inferenza. Sul percorso di successo esegue tre lookup per pacchetto, su \texttt{mac\_table}, \texttt{pkt\_stats} e \texttt{cls\_stats}: sono il costo di framework comune alle pipeline, non operazioni di inferenza. Questo riferimento permette di distinguere il costo del modello neurale da quello del solo framework XDP.
...
Memoria mappe (byte) & 280 & 2\,356 & 9\,356 & 18\,704 \\
\midrule
...
Memoria mappe (byte) & 13\,448 & 9\,356 & 27\,400 & 18\,704 \\
Latenza minima (ns/pacchetto) & 349.7 & 224.0 & 590.0 & 387.0 \\
\bottomrule
\end{tabular}
\end{table}

I risultati sono nella Tabella~\ref{tab:blocco-pesi}. Le letture di mappa per pacchetto scendono da $147$ a $8$ per P2 e da $160$ a $26$ per P3; le istruzioni e il codice jited si riducono sensibilmente. Anche la memoria occupata dalle mappe diminuisce, ma di circa un terzo: il 30,4\% per P2 e il 31,7\% per P3. Il layout a blocco concentra i pesi in un'unica entry, anziché distribuirli in migliaia di entry separate.

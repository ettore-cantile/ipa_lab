...
Cifra tipica (baseline) & $\approx 20$ ns, cioè 50 Mpps &
  $\approx 200$ ns, 2,1 Mpps misurati \\[3pt]
...
Riproducibile? & sì, è deterministico &
  la latenza sì, la portata no \\
\bottomrule
\end{tabular}
\end{center}

\begin{lezione}
I 50 Mpps della baseline non sono un throughput: sono il reciproco della durata
di una funzione. Un pacchetto che non viene mai consegnato non ha un throughput.
\end{lezione}

Il rapporto fra le due colonne --- $20$ ns contro $200$ --- non è un difetto del
secondo banco. È la risposta alla domanda \emph{quanto costa la rete intorno al
programma}, e su questa macchina la risposta è: nove volte il programma stesso.
...
virtuale, non di IPA.} È anche la ragione per cui i 50 Mpps stimati dalla durata
del programma non saranno mai misurabili qui: mancherebbe un tetto di venti
volte, e il tetto non è il programma.
...
\item \textbf{La portata non è ancora citabile.} La dispersione fra i giri
      arriva al 137\% sulle pipeline veloci, tutte limitate dal generatore. La
      latenza sì, ed è la misura su cui appoggiare le affermazioni di velocità
      finché questa resta così.
\item \textbf{Lo sweep sulle taglie di frame} (512 e 1514 byte) è impostato ma
      non ancora eseguito con il banco equo: finora solo 64 byte.

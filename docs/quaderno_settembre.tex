...
esce \textsc{crash} e lo sweep continua. Larghezza 16 invece si misura, ed è un
punto che Pipeline~2 e 3 non possono avere.
...
\midrule
p1\_static & 0,054 & 0,69 \\
hardcoded  & 0,061 & 0,75 \\
template   & 0,084 & 0,29 \\
modular    & 0,083 & \textbf{0,11} \\
\bottomrule
\end{tabular}
\end{center}
...
$n_{in}$ per quattro, pesi per 2,4, e la latenza \textbf{non si muove}. Le
istruzioni di Pipeline~1 intanto salgono da 1146 a 1702: la taglia statica
...
\midrule
p1\_static & 0,207 & \textbf{0,97} & 0,054 & 0,69 \\
hardcoded  & 0,224 & \textbf{0,95} & 0,061 & 0,75 \\
template   & 0,485 & 0,74          & 0,084 & 0,29 \\
modular    & 0,874 & \textbf{0,90} & 0,083 & 0,11 \\
\bottomrule
\end{tabular}
\end{center}

Per Pipeline~3 si passa da $r^2 = 0{,}11$ a $r^2 = 0{,}90$. Quattro assi
costruiti in modi diversi collassano sulla stessa retta, una per pipeline.

\begin{lezione}
\textbf{Il costo per MAC eseguita è una proprietà misurabile della pipeline}:
circa 0,21~ns quando i pesi sono compilati come letterali, circa 0,87~ns quando
vengono letti da una mappa. Un fattore $4\times$, ed è il prezzo della
genericità espresso in una costante invece che in un aneddoto.
\end{lezione}

Il pavimento della \texttt{baseline}, che di MAC ne esegue zero, sta fra 8 e
13~ns: è il costo del framework XDP che ogni pipeline paga prima di
moltiplicare qualunque cosa.

\begin{prima}[Un residuo che ha una spiegazione]
Il residuo medio di Pipeline~3 è più alto sull'asse della profondità (19~ns)
che sugli altri. È atteso: ogni strato in più è anche una \emph{tail call}, e
una tail call non è una MAC. Sull'asse delle istruzioni, invece, Pipeline~3 non
ha nemmeno una pendenza da stimare --- il suo conteggio è 12\,349 su ogni
cella, perché non ricompila mai, mentre la latenza varia da 218 a 444~ns. Il
suo costo non sta nella taglia del codice.
\end{prima}

% ------------------------------------------------------------------
\section{I due banchi non misurano la stessa cosa}

Resta da chiudere il cerchio. I due banchi a
\texttt{BPF\_PROG\_TEST\_RUN} misurano lo stesso programma, e il banco sul
percorso reale misura $T_2-T_1$. Danno lo stesso numero? No, e la differenza
non è una costante.

Sullo stesso modello 65-4-4-7:
...
La causa è il frame. La campagna riusa un solo pacchetto per centomila
ripetizioni e \texttt{BPF\_PROG\_TEST\_RUN} non ripristina il buffer fra una e
l'altra: il datapath decrementa il TTL, che dopo poche decine di ripetizioni
resta inchiodato a~1, e da lì in poi ogni esecuzione prende il ramo
\texttt{if (ip->ttl <= 1) return XDP\_PASS}. Quel ramo sta \emph{dopo}
l'inferenza --- il modello gira lo stesso, ed è il motivo per cui le curve
scalano correttamente --- ma salta la coda di inoltro: decremento, checksum,
\texttt{pkt\_stats}, \texttt{cls\_stats}, \texttt{mac\_table},
\texttt{bpf\_redirect}. \texttt{test\_suite} rinfresca il frame ogni 200
esecuzioni e quella coda la paga sempre.

\begin{lezione}
La campagna misura \textbf{l'inferenza}; \texttt{test\_suite} misura
\textbf{inferenza più inoltro}. Sono due grandezze diverse, non la stessa con
uno scostamento. Non vanno messe sulla stessa curva, e lo scarto è massimo
sulla \texttt{baseline} ($-61\%$) proprio perché per lei la coda di inoltro
\emph{è} quasi tutto il programma.
\end{lezione}
...
e diverge nella direzione giusta: \texttt{test\_suite} usa i pesi veri del
modello addestrato, la campagna pesi sintetici a sparsità nulla. I pesi veri
hanno qualche zero, clang cancella quelle moltiplicazioni, 45 istruzioni in
meno. È lo stesso effetto che l'asse \texttt{sparsity} misura apposta.
\end{prima}

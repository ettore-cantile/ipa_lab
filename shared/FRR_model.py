...
class QATFastRerouteMLP(FastRerouteMLP):
    ...
    Uso in train.py:
        from FRR_model import QATFastRerouteMLP
        model = QATFastRerouteMLP(n_interfaces=6, n_nodes=52, hidden_dim=4)

    Uso in esegui_pipeline.py:
        run_pipeline(quantization_method="qat", ...)
        # train.py deve scegliere il modello in base a questo flag
    """
    ...
    def _fake_quant(w: torch.Tensor) -> torch.Tensor:
        ...
        # round() con STE: gradiente = 1 (torch lo gestisce automaticamente)

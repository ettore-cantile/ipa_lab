// AUTO-GENERATED by gen_full_c.py (arch-faithful, descriptor-driven) -- do not edit by hand.
// dispatcher + tail-call + double-parse == BCC hardcoded architecture.
// descriptor: [link_state[6], ingress_iface[6], ttl[1], node[52]] -> 65-4-4-7
#include <linux/bpf.h>
...
int xdp_model(struct xdp_md *ctx) {
    ...
    { __u32 _z=0; struct link_state_vec *_p = bpf_map_lookup_elem(&link_state, &_z);
      if (_p) {
        ls0=(long long)_p->v[0];
        ls1=(long long)_p->v[1];
        ls2=(long long)_p->v[2];
        ls3=(long long)_p->v[3];
...
int xdp_dispatch(struct xdp_md *ctx) {
    ...
    bpf_tail_call(ctx, &model_progs, mid);
    return XDP_PASS;   // only if model_id has no registered program
}
